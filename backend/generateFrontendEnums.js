...
let opt1 = 'export const newSchoolCityOptions = [\n' + data1.map(c => '  { label: "' + c.replace(/"/g, '\\"') + '", value: "' + sanitize(c) + '" }').join(',\n') + '\n];\n';
...
let opt2 = 'export const schoolCountryOptions = [\n' + data2.map(c => '  { label: "' + c.replace(/"/g, '\\"') + '", value: "' + sanitize(c) + '" }').join(',\n') + '\n];\n';

fs.writeFileSync('../frontend/src/helpers/schoolEnums.js', opt1 + '\n' + opt2);
